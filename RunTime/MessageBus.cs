...
namespace AshenLadd.Messaging
{
    ...
    public static class MessageBus
    {
        ...
        public static void RemoveListener<T1>(string eventName, Action<T1> callback) =>
            RemoveInternal(eventName, callback);
        ...
        private static void AddInternal(string eventName, Delegate callback)
        {
            ...
            if (ListenersDictionary.TryGetValue(eventName, out var existingDelegate))
            {
                ...
                {
                    Debug.LogWarning(
                        $"[MessageHub] Listener type mismatch for event '{eventName}'. Expected {existingDelegate.GetType().Name}, got {callback.GetType().Name}.");
                    return;
                }
                ...
                foreach (Delegate existing in existingDelegate.GetInvocationList())
                {
                    ...
                    {
                        Debug.LogWarning(
                            $"[MessageHub] Listener already subscribed for event '{eventName}'. Not adding again.");
                        return;
                    }
                }
        ...
        public static void Broadcast<T1, T2, T3>(string eventName, T1 param1, T2 param2, T3 param3,
            object sender = null)
        {
